...
%[p,Fs2]=audioread('audioprocessedtest3.wav');
%plot(y);
frame_sync='0011111010';
u=lowpass(y,200,48000);
y1=u(557207:1026196);
Ts=1/Fs1;
...
[ZeroValue,datavalues,averageMax,averageMin,maxValue,minValue]=...
bucketData(y1);
[averageTransition,threshold,transitionPoint]=...
recoverClockOffset(averageMax,averageMin,ZeroValue,datavalues);
clockAdvance=recoverClock(averageTransition);
y2=u((557207+clockAdvance):1026196+clockAdvance);
t=repmat(y2(1:432000),30,1);
amp=0.1; %By changing this we can change the noise amplitude added to the set
noise =amp*randn(size(t));
t=t+noise;
[ZeroValue1,datavalues1,averageMax1,averageMin1]=bucketData1(t);
[middleSample2,b1]=sampleBuckets(datavalues1,averageMax1,...
    averageMin1,ZeroValue1);
str='9'; %This is simply an initialization.
for i=1:length(middleSample2)
    s(i)=string(middleSample2(i));
    str=strcat(str,s(i));
end
str1=convertStringsToChars(str);
k=strfind(str1,'0011111010');

function [ZeroValue,datavalues,averageMax,averageMin,maxValue,...
    minValue] = bucketData(dataValue)
k=1;
sample_window_length=1800;
sample_bucket_length=240;
maxValue=zeros(1,sample_window_length);
...
function [ZeroValue,datavalues,averageMax,averageMin,maxValue,...
    minValue] = bucketData1(dataValue)
k=1;
sample_window_length=54000;
sample_bucket_length=240;
maxValue=zeros(1,sample_window_length);
minValue=zeros(1,sample_window_length);
averageMax=0;
...
function [averageTransition,threshold,transitionPoint] =...
    recoverClockOffset(averageMax,averageMin,zeroValue,datavalues)
...
sample_bucket_length=240;
clock_tolerance=20;
if(averageTransition>sample_bucket_length/clock_tolerance&&...
        averageTransition<...
        sample_bucket_length-sample_bucket_length/clock_tolerance)
    clockAdvance=(averageTransition-...
        sample_bucket_length/clock_tolerance);
else
    clockAdvance=0;
end
end
function [middleSample,b] = ...
    sampleBuckets(datavalues,averageMax,averageMin,ZeroValue)
...
for i=1:sample_window_length
    ...
    samples=0;
    e=1;
    for s=((sample_bucket_length/2)-sample_width):1 ...
        :((sample_bucket_length/2)+sample_width)
